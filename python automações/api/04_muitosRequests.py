...
def busca_id_estado():
    """Obtém um dicionario de estados no formato {id_estado}"""
    url = "https://servicodados.ibge.gov.br/api/v1/localidades/estados"
    dados_estado = obter_request(url, params={"view": "nivelado"}) or []
    return {estado["UF-id"]: estado["UF-nome"] for estado in dados_estado}

def frequencia_nome(name):
    """Obtém um dicionario de frequência de um nome por estado no formato {id_estado: frequencia}"""
...
def main(name):
    ...
    print(f"=== Frequẽncia do nome {name} nos Estados (por 100.000 habitantes) ===")
